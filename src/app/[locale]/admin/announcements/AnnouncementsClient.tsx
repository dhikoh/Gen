...
export default function AnnouncementsClient() {
 ...
 <p className="text-sm pg-text-muted">{t("pageDesc")}</p>
 </div>

 <div className="pg-surface border pg-border rounded-xl p-6 shadow-sm">
 <h2 className="text-lg font-semibold pg-text-heading mb-4">{t("createTitle")}</h2>
 ...
 className="w-full px-3 py-2 border pg-border rounded-lg bg-white pg-text-heading focus:outline-none focus:ring-2 focus:ring-blue-500"
 required
 />
 </div>
 ...
 <div className="pg-surface border pg-border rounded-xl p-6 shadow-sm">
 <h2 className="text-lg font-semibold pg-text-heading mb-4">{t("historyTitle")}</h2>
 {loading ? (
 <div className="py-8 text-center pg-text-muted">{t("loading")}</div>
 ) : announcements.length === 0 ? (
 <div className="py-8 text-center pg-text-muted">{t("empty")}</div>
 ) : (
 <div className="space-y-4 divide-y pg-divide">
 {announcements.map((item, idx) => (
 <div key={item.broadcastGroupId || idx} className="pt-4 first:pt-0 space-y-1">
 <div className="flex items-center justify-between">
 <h3 className="font-semibold pg-text-heading">{item.title}</h3>
 <div className="text-right">
 <span className="text-xs pg-text-muted block">
 ...
 <p className="text-sm pg-text-sub dark:pg-text-muted whitespace-pre-line">{item.message}</p>
 {item.link && (
 <a
 href={item.link}
 target="_blank"
