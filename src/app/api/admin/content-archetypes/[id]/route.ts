...
export async function PUT(
  ...
  const isAllowed = await applyRateLimit(`admin_archetypes_put_${session.user.id}_${ip}`, 20, 60);
  if (!isAllowed) {
  ...
  try {
    ...
    await logAdminAction({
      ...
      action: "UPDATE_CONTENT_ARCHETYPE",
...
export async function DELETE(
  ...
  const ip = getClientIp(req);
  const isAllowed = await applyRateLimit(`admin_archetypes_delete_${session.user.id}_${ip}`, 20, 60);
  if (!isAllowed) {
    return NextResponse.json({ error: t("tooManyRequests") }, { status: 429 });
  }
  ...
  try {
    ...
    await logAdminAction({
      adminId: session.user.id,
      action: "DELETE_CONTENT_ARCHETYPE",
      targetType: "CONTENT_ARCHETYPE",
      targetId: id,
