...
export default function ForgotPasswordForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        setMessage({ type: "error", text: data.error || "Gagal mengirim tautan reset." });
      } else {
  ...
  return (
    <div className="w-full max-w-md mx-auto p-6 bg-white dark:bg-zinc-900 rounded-xl shadow-xl border border-zinc-200 dark:border-zinc-800">
      ...
      <h2 className="text-xl font-bold text-zinc-900 dark:text-white mb-2">Lupa Password?</h2>
      <p className="text-sm text-zinc-500 dark:text-zinc-400 mb-6">
        Masukkan email atau username Anda. Kami akan mengirimkan tautan untuk mengatur ulang password Anda.
      </p>
